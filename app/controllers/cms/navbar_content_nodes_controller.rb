...
module Cms
  class NavbarContentNodesController < ApplicationController
    def index
      @content_nodes = unscoped.used_in_navbar
    end

    def select
      @content_nodes_for_selection = Cms::ContentNode.not_used_in_navbar.where(parent_id: nil).order(:title).pluck(:title, :id)
    end
    ...
    def positions
      ids = Cms::ContentNode.currently_in_navbar.pluck(:id)
    ...
    before_action :load_object, only: [:add, :remove, :toggle_access]
    ...
    def load_object
      @content_node = Cms::ContentNode.unscoped.find(params[:id])
    end
